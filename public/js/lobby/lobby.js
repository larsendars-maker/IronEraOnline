...
async function refreshRooms(){
  try{
    ...
    document.querySelectorAll('.delete-room').forEach(b=>b.onclick=async()=>{
      ...
      if(!dd.ok){toast(dd.error,'error');return;}
      await refreshRooms();
    });
  }catch{rooms.innerHTML='<div class="empty">Не удалось загрузить список комнат.</div>';}
}
createRoom.onclick=()=>{if(!getToken()){showAuth();return;}if(createRoom.disabled){toast('У тебя уже есть комната. Сначала удали её.','error');return;}roomModal.classList.remove('hidden');roomName.focus();};
